...
class HomePageState extends ConsumerState<HomePage> {
  ...
  void _registerNotification() {
    ...
    _firebaseMessaging.getToken().then((token) {
      final _homeProvider =
        ref.watch(homeProvider.notifier);
      print('push token: $token');
      if (token != null) {
        ref.watch(authProvider).whenData((userChat) {
          if (userChat != null) {
            _homeProvider.updateDataFirestore(FirestoreConstants.pathUserCollection,
                userChat.id, {'pushToken': token});
          }
        });
      }
  ...
  Future<void> _handleSignOut() async {
    await ref.read(authProvider.notifier).handleSignOut();
  ...
  Widget build(BuildContext context) {
    ...
    useEffect(() {
      _authProvider.whenData((userChat) {
        if (userChat == null) {
          context.pushReplacement(Routes.loginPage);
        }
      });
  ...
  Widget _buildItem(DocumentSnapshot<Map<String, dynamic>>? document) {
    if (document != null) {
      final userChat = UserChat.fromJson(document.data()!);
                      ...
                      child: Container(
                        child: Column(
                          children: [
                            ...
                            Container(
                              child: Text(
                                'About me: ${userChat.aboutMe ?? "Nice to meet you!"}',
                                maxLines: 1,
                                style:
                                    TextStyle(color: ColorConstants.primaryColor),
                              ),
